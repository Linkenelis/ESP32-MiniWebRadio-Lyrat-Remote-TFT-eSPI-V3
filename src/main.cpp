...
#define AA_FONT_SMALL 20    //headline etc
...
void showFileName(const char* fname){
    ...
    if(strlen > 45) {str=str.substring(strlen-45);}    //limit to 1 line, we want last part
    UDP.beginPacket(Display, UDP_port); //open
    if(fontLoaded!=AA_FONT_SMALL)
    {   
        UDP.printf("tft.loadFont(%d)\n",AA_FONT_SMALL);
...
void connecttoFS(const char* filename, uint32_t resumeFilePos){
    ...
    webSrv.send("audiotrack=" + _audiotrack.substring(_audiotrack.lastIndexOf("/") + 1));
...
inline void setVolume(uint8_t vol){
    ...
    if(HP_DETECT == -1){    //Not selected
        dac.outputSelect(OUTALL);
        dac.setOutputVolume(_cur_volume *3); //vol max is 33, vs 21.several ways to do this *1.55 very small steps low vol
    }
    else{
        if(digitalRead(HP_DETECT) ==  HIGH){    //speaker
            // SerialPrintfln("HP_Detect = High, volume %i", vol);
            dac.outputSelect(OUT2);
            dac.setOutputVolume(_cur_volume *3);
        }
        else {  //headphones
            // SerialPrintfln("HP_Detect = Low, volume %i", vol);
            dac.outputSelect(OUT1);
            dac.setOutputVolume(_cur_volume *3);
        }
    }  
}
...
void audiotrack(const char* fileName, uint32_t resumeFilePos){
    ...
    changeState(PLAYER);
    if(plays==SDcard){connecttoFS((const char*) path, resumeFilePos);}
    if(_f_isFSConnected){
...
void get_nbr_of_tracks(void){
    File trcklst;
    char ch;
    nbroftracks=0;
    if(plays==SDcard){trcklst = SD_MMC.open("/tracklist.txt", FILE_READ);} //open file for reading
    if(plays==DLNA){trcklst = SD_MMC.open("/DLNAlist.txt", FILE_READ);}
...
void next_track(int tracknbr)
  {
    ...
    int trcknbr=0;
    File trcklst;
    if (nbroftracks<=1){     //get nbr of tracks (only once) TODO maybe not once
        get_nbr_of_tracks();
    }
    Serial.print("nbr tracks ");Serial.println(nbroftracks);
    trcklst.close();
    if(tracknbr==1000000) {trcknbr = random(1, nbroftracks+1);}
    else {trcknbr=tracknbr; if(trcknbr>nbroftracks) {trcknbr=1;} if(trcknbr<1){trcknbr=nbroftracks;}} // end to start and vv
    //Serial.print("We have the next trcknbr = ");Serial.println(trcknbr);  //we have the track number

    //get the number and name of that track
    sstr=String(trcknbr)+"\t:";
    //Serial.print("Searching for tracknumber ");Serial.println(sstr);  // search for tracknumber\t:
    if(plays==SDcard){trcklst = SD_MMC.open("/tracklist.txt", FILE_READ);name_end= '\n';} //open file for reading
    if(plays==DLNA){trcklst = SD_MMC.open("/DLNAlist.txt", FILE_READ);name_end=';';}
    while (trcklst.available())
    {
      ch = trcklst.read();
      _audiotrack += ch;
      if (_audiotrack == sstr) {found = true; _audiotrack = "";} //search for the track nbr, empty _audiotrack if the track is found
      while (found)
      {
        ch = trcklst.read();
        Serial.print(ch);
        if (ch == name_end) //this is what we want
        {
        ...
        {
          _audiotrack += ch;    //add chars until new line, while found = true 
        }
      }  
...
void next_audio_tracknbr(bool prevnext)  //1=next; 0=prev
  {
    ...
    _audiotrack="";
    //Serial.print("current track="); Serial.println(sstr);
    //get the number and name of that track
    if(plays==SDcard){trcklst = SD_MMC.open("/tracklist.txt", FILE_READ); name_end= '\n';} //open file for reading
    if(plays==DLNA){trcklst = SD_MMC.open("/DLNAlist.txt", FILE_READ); name_end=';';}
    while (trcklst.available())
    {
        ...
        if (ch == '\n') //new line'; delete track, while not found
        {
            ...
            if (ch == ':'){ //yes, now we have '\t:'
                tracknbr=atoi(track.c_str());  //store the number of the track being read
                track="";   //now we read the track name, so clear track
            }     
        }
        else {track += ch;}
        if(plays==DLNA){if (ch==name_end){track="";}}   //for DLNA search for sstr starts now
        // now find the path to the current track
        if (track == sstr) {found = true; track = "";} //found current track in list and we have it's number; send next or previous to next_track
        while (found)   
        {
            found = false;
            if(prevnext){   //next
                //Serial.print("tracknbr=");Serial.println(tracknbr+1);
                next_track(tracknbr+1);
                trcklst.close();
                return;
            }
            else{   //previous
...
void next_track_needed(bool prevnext){  //1=next; 0=prev
    if (mp3playall)
    {
        if (millis() - previousMillis >= 1000)
        {
            ...
            if(plays==DLNA){connectto = _audiotrack;}
            Serial.print("connectto= ");
            Serial.println(connectto);
            audioStopSong();
            if(plays==DLNA){DLNA_getFileItems(_audiotrack);}
            if(plays==SDcard){audiotrack(connectto.c_str(), 0);}
            str = "audiotrack=Playing track: ";
...
bool send_tracks_to_web(void){  //read tracklist and send is to the webpage
    ...
    while (trcklst.available())
    {
        ...
        if (ch == '\n') //new line'; send the track name and delete track
        {
            ...
            jObject[i]["name"] = track;
            track = "";
            if(i>35){break;}        //is more than 35 usefull?
            i++;
        }
...
void DLNA_showContent(String objectId, uint8_t level){
    ...
    if(objectId.startsWith("D=")) {
        ...
        DLNA_browseServer(objectId, level);
        File DLNAtracks;
        SD_MMC.remove("/DLNAlist.txt");
        DLNAtracks = SD_MMC.open("/DLNAlist.txt", FILE_WRITE); //create a new file during (new) Directory read
        DLNAtracks.close();
        nbroftracks=0;
    }
    if(objectId.startsWith("F=")) {
        objectId = objectId.substring(2);
        _audiotrack=objectId;
        mp3playall=true;
    ...
    if (shortcounter >=10) {    //1 sec
        ...
        if(_f_rtc==true){ // true -> rtc has the current time
            ...
            if(_state == PLAYER){
                ...
                //webSrv.send("_audiotrack="+artsong);
            }
...
void WEBSRV_onCommand(const String cmd, const String param, const String arg){                       // called from html
    ...
    if(cmd == "audiotrack"){        plays=SDcard; String msg="getplays=" + String(plays) ;webSrv.send(msg); nbroftracks=0; connectto=""; _audiotrack=""; audiotrack(param.c_str()); return;}
    ...
    if(cmd == "stopfile"){              _resumeFilePos = audioStopSong(); webSrv.reply("OK\n"); return;}
    ...
    if(cmd == "artsong"){           webSrv.send("audiotrack="+artsong); return;}
    ...
    if(cmd == "DLNA_getContent0"){ plays=DLNA; String msg="getplays=" + String(plays) ;webSrv.send(msg); connectto=""; _audiotrack=""; _level = 0; DLNA_showContent(param, 0); return;}
    ...
    if(cmd == "DLNA_getContent6"){  _level = 6; DLNA_showContent(param, 6); return;} // search for level 6 content
    if(cmd == "DLNA_getContent7"){  _level = 7; DLNA_showContent(param, 6); return;} // search for level 7 content
...
void dlna_file(String name, String id, size_t size, String uri, bool isAudio){ // list files
    File DLNAtracks;
    String msg;    
    if(isAudio) {SerialPrintfln("DLNA_file:   " ANSI_ESC_GREEN "\"%s\"", name.c_str()); nbroftracks++; msg=String(nbroftracks); msg.concat("\t:"); msg.concat(name); msg.concat(";"); msg.concat(uri); msg.concat("\n"); DLNAtracks = SD_MMC.open("/DLNAlist.txt", FILE_APPEND); DLNAtracks.print(msg); DLNAtracks.close();}
    ...
    msg = "Level" + String(_level + 1,10) + "=" + JSON.stringify(myObject); //10 levels
    webSrv.send(msg);
}
